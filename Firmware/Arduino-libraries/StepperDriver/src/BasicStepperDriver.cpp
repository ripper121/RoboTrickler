...
#define I2S_BEEPER                  7
ShiftRegister74HC595<1> sr(I2S_OUT_DATA, I2S_OUT_BCK, I2S_OUT_WS);
...
long BasicStepperDriver::nextAction(void){
    if (steps_remaining > 0){
         ...
         */
        sr.set(dir_pin, dir_state);
        sr.set(step_pin, HIGH);
        unsigned m = micros();
...
void BasicStepperDriver::enable(void){
    if IS_CONNECTED(enable_pin){
        sr.set(enable_pin, enable_active_state);
    };
    delayMicros(2);
}
...
void BasicStepperDriver::disable(void){
    if IS_CONNECTED(enable_pin){
        sr.set(enable_pin, (enable_active_state == HIGH) ? LOW : HIGH);
    }
}
...
void BasicStepperDriver::beep(unsigned long time){
    sr.set(I2S_BEEPER, HIGH);
    delay(time);
    sr.set(I2S_BEEPER, LOW);
}
