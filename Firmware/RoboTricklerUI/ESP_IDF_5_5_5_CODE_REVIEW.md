...
- Completion-count persistence snapshots and marks the same protected value, and Wi-Fi configuration changes are rejected while the runtime or a profile test is active.
- Web status uses one spinlock-protected state/weight snapshot. Blocking dialog completion flags use `std::atomic<bool>`.
- Firmware version comparison validates every segment before using the comparison result, and the update check consumes a body only after `HTTP_CODE_OK`.
...
| `trickler_control.ino`, `trickler_runtime.ino`, `scale_rs232.ino` | Cross-core critical sections, task blocking, UART input limits, I2S motion calls | State, web weight snapshots, and completion-counter persistence use the shared spinlock. Scale framing and calibration are application contracts. |
...
| `ota_update.ino`, `firmware_check.ino` | OTA slots, update validation, HTTP client | Early SD OTA is the intended recovery path. The version response is bounded, fully validated, and accepted only with HTTP 200. |
...
| `web_server.ino`, `web_api.ino`, `web_file_editor.ino` | HTTP server, static files, update streaming, heap | Static file downloads and unauthenticated routes are intentional. Asynchronous Wi-Fi scan releases results with `WiFi.scanDelete()`; runtime status is built from one synchronized snapshot. |
| `ui.c`, `ui_Screen1.c`, `ui.h`, `ui_events.h`, `ui_fonts.h`, `ui_events.ino`, `ui_dialogs.ino`, `ui_text.ino` | LVGL calls, UI/task synchronization, filesystem-backed language loading | Generated UI and widget settings are LVGL contracts. Language loading uses the filesystem guard, and cross-task dialog signals are atomic. |
...
- Rechecked the installed core's `Update/examples/SD_Update` and `Update/examples/OTAWebUpdater`, `HTTPClient/examples/StreamHttpClient`, and local LVGL Arduino example; checked the supplied IDF I2S, watchdog, OTA, Wi-Fi, filesystem, FreeRTOS, HTTP, GPIO, and heap guides against the relevant modules. The project uses the installed wrapper APIs.
- All local documentation links in this report resolve. The project instruction's `tools/compile_upload.py` is absent, so the current equivalent `python tools/firmware_build_upload.py --cli --error --compile-only` was run successfully: firmware uses 1.49 MiB of the 3.19 MiB app slot and the generated LittleFS files use 477.9 KiB of the 1.50 MiB image. The only compiler/linker diagnostic is the bundled toolchain's `_fixdfdi.o` missing `.note.GNU-stack` warning. All four JSON files under `SD-Files` parse successfully. Device behavior remains unverified.
