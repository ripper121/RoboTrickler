...
- During operation, touchscreen target-weight controls are replaced by a full-width display of the live remaining delta (`targetWeight − scale weight`) and the calculated weight of the next bulk or fine trickle; disabled tab captions are hidden. The web interface disables profile selection while running; target/profile API mutations are rejected during a run.
