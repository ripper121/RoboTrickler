...
void handleCalibrationProfilePrompt()
{
    ...
    if (newData && (lastScaleWeightReadTime > calibrationProfilePromptTime))
    {
        ...
        newData = false;
        weightCounter = 0;
        if (confirmBox(String(languageText("msg_create_profile_prompt")) + String(weight, 3) + " gn", &lv_font_montserrat_14, lv_color_hex(0xFFFFFF)))
        {
            String profileName = "";
            if (createProfileFromCalibration(weight, profileName))
            {
                messageBox(String(languageText("msg_profile_created")) + profileName + ".txt", &lv_font_montserrat_14, lv_color_hex(0x00FF00), true);
            }
            else
            {
                messageBox(languageText("msg_create_profile_failed"), &lv_font_montserrat_14, lv_color_hex(0xFF0000), true);
            }
        }
    }
}
