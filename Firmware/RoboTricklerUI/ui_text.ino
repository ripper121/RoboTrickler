...
static const LangFallbackEntry LANG_FALLBACKS[] = {
    ...
    {"status_saving_config_failed", "Configuration save failed!"},
    ...
    {"err_entry", "\nEntry: "},
    {"err_required_profile_entry", "\nRequired: diffWeight, measurements, stepper.steps"},
    ...
    {"msg_tune_limit_factor_title", "Map limit factor"},
...
bool loadLanguage()
{
  ...
  // Parse straight from the file stream and retain only the section used by
  // langText(). The same JSON format may gain other consumers later; they must
  // not increase the UI language document's resident heap usage.
  JsonDocument filter;
  filter["ui"] = true;
